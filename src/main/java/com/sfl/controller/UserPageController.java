...
public class UserPageController {
	...
	@RequestMapping(value = "/mytables", method = RequestMethod.GET)
	public ModelAndView showMyTable(@RequestParam int id) {
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.addObject("rule", new User());
		modelAndView.addObject("user", userService.findUser(id));
		modelAndView.addObject("waitertables", waiterTableService.findByUser(userService.findUser(id)));
		modelAndView.addObject("mode", "MODE_TABLE");
		modelAndView.addObject("auth", getUser());
		modelAndView.addObject("control", getUser().getRole().getRole());
		modelAndView.setViewName("user_profile");
		return modelAndView;
